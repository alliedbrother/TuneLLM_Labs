...
export default function PlaygroundPage() {
  ...
  const deployedModels = modelsData?.items.filter((m) => m.status === 'deployed') || [];

  const [selectedModel, setSelectedModel] = useState(deployedModels[0]?.id?.toString() || '');
  ...
  const handleGenerate = async () => {
    ...
    // Simulate response - in real implementation, this would call the inference API
    setTimeout(() => {
      setResponse(
